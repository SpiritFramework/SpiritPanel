  ...
  return (
    ...
    >
      ...
      <Link to={`/servers/${server.id}`} className="relative flex flex-1 flex-col">
        ...
        <div className={`relative flex flex-1 flex-col ${cardStyle === 'banner' ? '-mt-2.5 px-4 pb-4' : 'px-4 pb-4 pt-1'}`}>
          <CardBody
            server={server}
            address={address}
            copied={copied}
            flat={cardStyle === 'minimal'}
...
function CardHeader({
  ...
  const iconBox = (
    <div
      ...
      style={cardStyle === 'banner' ? undefined : { borderColor: theme.accent }}
    >
    ...
    <div className="min-w-0 flex-1">
      <p
        className={`text-[10px] font-semibold uppercase tracking-wider ${
          cardStyle === 'banner' ? 'text-white/55' : 'text-[var(--muted)]'
        }`}
      >
        {theme.label}
      </p>
      <h3
        className={`mt-0.5 line-clamp-2 font-semibold leading-snug ${
          cardStyle === 'banner' ? 'text-base text-white' : 'text-base text-[var(--text)]'
        }`}
      >
        {server.name}
      </h3>
      {server.description && (
        <p
          className={`mt-1 line-clamp-1 text-xs ${
            cardStyle === 'banner' ? 'text-white/60' : 'text-[var(--muted)]'
          }`}
        >
          {server.description}
        </p>
  ...
  if (cardStyle === 'stacked') {
    return (
      <div className="server-card-stacked-head px-4 pt-4 text-center">
        ...
        <p className="text-[10px] font-semibold uppercase tracking-wider text-[var(--muted)]">{theme.label}</p>
        <h3 className="mt-1 line-clamp-2 text-base font-semibold leading-snug">{server.name}</h3>
        {server.description && (
          <p className="mt-1 line-clamp-2 text-xs text-[var(--muted)]">{server.description}</p>
        )}
      </div>
    );
  }
